    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between">
        <div>
          ...
          <h2 className="font-specimen text-3xl font-bold tracking-tight">{t("orders.title")}</h2>
        </div>
        <span className="specimen-latin">{t("orders.records", { total })}</span>
      </header>
      ...
      >
        ...
        >
          {STATUS_OPTIONS.map((s) => (
            <option key={s} value={s}>{s ? t(ORDER_STATUS_KEY[s]) : t("orders.allStatus")}</option>
          ))}
        </select>
        <button type="submit" className="btn-amber rounded-xl px-5 py-2.5 text-sm font-medium">{t("orders.search")}</button>
        {(keyword || status) && (
          <button
            type="button"
      ...
      <div className="specimen-card overflow-hidden !py-0">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border/70 text-left">
              {["orders.colNo", "orders.colStatus", "orders.colAmount", "orders.colSku", "orders.colTime", "orders.colOp"].map((hk) => (
          ...
          <tbody>
            {orders.map((o, i) => (
              <tr key={o.id} className="row-in border-b border-border/40 last:border-0 hover:bg-muted/40" style={{ animationDelay: `${i * 30}ms` }}>
                ...
                <td className="px-5 py-3.5 text-muted-foreground">{t("orders.items", { count: o.items.length })}</td>
                <td className="px-5 py-3.5 text-xs text-muted-foreground">{fmtTime(o.created_at)}</td>
                <td className="px-5 py-3.5">
                  <button className="rounded-lg border px-2.5 py-1 text-xs hover:bg-muted" onClick={() => setDetail(o)}>{t("orders.detail")}</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {!isLoading && orders.length === 0 && (
          <p className="p-5 text-sm text-muted-foreground">{t("orders.noMatch")}</p>
        )}
      </div>
      <Pagination total={total} page={page} pageSize={PAGE_SIZE} onChange={setPage} />

      {/* 详情弹窗 */}
      <Modal open={!!detail} title={t("orders.detailTitle", { no: detail?.order_no ?? "" })} onClose={() => setDetail(null)}>
        {detail && (
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-xl bg-muted/50 p-4">
              <div>
                <span className="specimen-latin block">status</span>
                <span className="text-sm font-medium">{t(ORDER_STATUS_KEY[detail.status] ?? detail.status)}</span>
              </div>
            ...
            {detail.remark && (
              <div className="rounded-xl border p-4 text-sm text-muted-foreground">{t("orders.remark", { remark: detail.remark })}</div>
            )}
            <div>
              <span className="specimen-latin mb-2 block">{t("orders.itemsTitle")}</span>
            ...
            <p className="text-right text-xs text-muted-foreground">{t("orders.colTime")} {fmtTime(detail.created_at)}</p>
          </div>
        )}
      </Modal>
    </div>
