...
- **批量异步入库**（v2，✅ `queen/app/tasks/crop_tasks.py` + `queen/app/api/v1/endpoints/crop.py`）：`POST /crop/documents/batch`（multipart 多文件）只做「提取文本 + register_document 建 **queued** 行」即回 **202**，向量化由 Celery `crop.ingest_batch` 串行消化（单 task 内逐文档，ack-agent 同款串行哲学防并发 embedding）；状态机 queued→embedding→ready/failed（`crop_service.ingest_document` 幂等：非 queued 跳过防重复投递）；单文件失败只标 failed 继续下一个；单文档同步快路径 = register_document+ingest_document（行为不变，测试契约不破）；进度聚合 `GET /crop/documents/stats`（ready/queued/embedding/failed/pending）；Wing 批量导入 Modal + 2s 轮询徽标/计数条。**自愈与重试**：worker `worker_ready` 信号启动清扫（`_sweep_stale_documents`：queued 全收 + embedding 超 30min 重置 queued 重入队，清扫失败只告警不阻断启动）；`POST /crop/documents/{id}/retry`（failed→queued 手动重试，非 failed 409）+ Wing failed 行 ↻ 按钮。PARITY 三端点（batch/stats/retry）走 EXEMPT（理由 test_capability_parity.py）
- **权限分域**（v0.11.1+，✅ `queen/app/services/crop_service.py:257` _visible_cond）：`visible_roles`（角色 key CSV，空=全库共享，admin 恒全量）；**检索前过滤**——向量通道扩召回 top_k×3 回表过滤、关键词通道 join 过滤；投影不存权限（权威/投影分离不变）；REST/MCP/agent 三方同源；红线测试 `queen/tests/test_crop.py` test_crop_visibility_scoping
- **混合检索**（v0.11+，✅ `queen/app/services/crop_service.py` search）：向量 KNN + MySQL 关键词 LIKE 召回（查询拆短语 ≥2 字，通配符转义）→ **RRF 融合**（k=60，score=RRF 分）；纯向量对编号/术语类查询弱，关键词兑底后**投影丢失不再全盲**（回归 `queen/tests/test_crop.py` test_crop_hybrid_search_keyword_fallback）
