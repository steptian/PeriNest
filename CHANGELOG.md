...
- **多语言 i18n Wing 业务页（第二批）**：Users + Roles 全量 t() 化——成员表格/禁用确认/创建·编辑弹窗
  （含权限覆盖面板）/角色矩阵表/守卫规则/角色表单（无→只读→读写三态循环）；角色名等 DB 数据不硬翻
...
- 问答 prompt 收紧：回答首字符即正文，禁止「我先检索一下」等元描述开场
  （检索过程 UI 已实时展示，模型无需复述）

### Changed
- **AI 交互合并**（三端）：AI 助手单入口 + 「引用知识库」开关（勾选=知识库
  问答带引用，不勾=自由对话；记住选择）+ 搜索模式（直查原文）；Leg/Antenna
  的知识库独立页删除，入口收敛
- **Wing 知识库页三 tab 分离**（管理/问答/试验互不混排）：`pages/crop/` 拆
  目录（index 容器 + manage/ask/lab）——管理=文档表+上传+重建，问答=左栏
  列表式历史会话 + 完整消息时间线（可回溯续聊/重命名），试验=检索命中测试；
  三 tab 共享固定视口高度面板内各自滚动，切换不跳动；表格状态中文化
